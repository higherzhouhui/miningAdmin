...
import { history, request, useLocation } from 'umi';
import * as XLSX from 'xlsx';
import { PlusCircleOutlined, SearchOutlined, TableOutlined } from '@ant-design/icons';
...
const TableList: React.FC = () => {
  ...
  const routeToChildren = (record: TableListItem) => {
    localStorage.setItem('followObj', JSON.stringify({ id: record.id, name: record.nick_name }));
  ...
  useEffect(() => {
    const obj = localStorage.getItem('followObj')
    if (obj) {
  ...
  return (
    <PageContainer>
      ...
      <ProTable<TableListItem, TableListPagination>
        ...
        id="accountListIndex"
        headerTitle={searchId ? `${searchName} 的关注列表：${total}` : ''}
        toolBarRender={() => [
          // <Button
          //   type="link"
          //   key="add"
        ...
        request={async (params: TableListPagination) => {
          const res: any = await listReq({ ...params, pageNum: params.current, user_id: searchId });
          let data: any = [];
          data = res?.data?.rows;
