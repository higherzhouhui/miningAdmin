...
const TableList: React.FC = () => {
  ...
  const routeToChildren = (record: TableListItem) => {
    localStorage.setItem('chatObj', JSON.stringify({ id: record.id, name: record.nick_name }));
  ...
  useEffect(() => {
    const obj = localStorage.getItem('chatObj')
    if (obj) {
  ...
  return (
    <PageContainer>
      ...
      <ProTable<TableListItem, TableListPagination>
        ...
        id="accountListIndex"
        headerTitle={searchId ? `${searchName} 的视频列表：${total}` : ''}
        toolBarRender={() => [
          // <Button
          //   type="link"
          //   key="add"
